 ...
 */
struct map_reduce {
	pthread_t* t;	//map&reduce thread
	pthread_t r;
	int tott;	//total number of threads
	map_fn map;	//for map
	reduce_fn reduce; //for recuce
	int* maps;
	int rs;	//for map and reduce, record if the mapping fail before even joing threads
	int* sockfd_c; //socket for client
	int sockfd_s; //socket for server
